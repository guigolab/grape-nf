...
| [red]#{counter:index-qs}# | id          | the run identifier (e.g. `test1`)
| [red]#{counter:index-qs}# | path        | the path to the fastq file
| [red]#{counter:index-qs}# | type        | the type (e.g. `bam`)
| [red]#{counter:index-qs}# | view        | an attribute that specifies the content of the file (e.g. `GenomeAlignments`)
| [red]#{counter:index-qs}# | readType    | the input data type (either `Single-End` or `Paired-End`)
| [red]#{counter:index-qs}# | readStrand  | the inferred exepriment strandednes if any (it can be `NONE` for unstranded data, `SENSE` or `ANTISENSE` for single-end data, `MATE1_SENSE` or `MATE2_SENSE` for paired-end data.)
|===
